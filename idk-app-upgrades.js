(() => {
  ...
  function richNotes() {
    ...
    const persist = () => { write('idkRichNotes', notes); write('idkNotes', notes.find(note => !note.trashed)?.text || ''); window.dispatchEvent(new CustomEvent('idk-data-changed', { detail: { type: 'notes', key: 'idkRichNotes' } })); };
    ...
    const renderEditor = () => {
      ...
      title.oninput = tags.oninput = area.oninput = delayed;
       const saveFile = el('button', { className: 'btn tab', type: 'button', textContent: 'Save to Files' });
       saveFile.onclick = () => { save(); window.IDKFiles?.writeTextFile?.(`${note.title || 'Untitled note'}.txt`, note.text, '', 'text/plain'); notify('Notes', 'The note was saved to Files.'); };
      ...
      trash.onclick = () => { note.trashed = !note.trashed; note.updated = Date.now(); persist(); folder = note.trashed ? 'trash' : 'all'; render(); };
       editor.replaceChildren(el('div', { className: 'rich-note-toolbar' }, [title, tags, status, saveFile, trash]), area);
    };
  ...
  function calendarUpgrade() {
    ...
    const persist = () => { write('idkCalendarEvents', events); window.dispatchEvent(new CustomEvent('idk-data-changed', { detail: { type: 'calendar', key: 'idkCalendarEvents' } })); };
