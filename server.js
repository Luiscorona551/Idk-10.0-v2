...
app.use((req, res, next) => {
  ...
  res.setHeader('Content-Security-Policy', "default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'self'; form-action 'self' https:; script-src 'self' 'unsafe-inline' 'unsafe-eval' 'wasm-unsafe-eval' https: blob:; style-src 'self' 'unsafe-inline' https:; img-src 'self' data: blob: https:; media-src 'self' data: blob: https:; font-src 'self' data: https:; connect-src 'self' https: ws: wss: blob:; frame-src 'self' https: data: blob:; worker-src 'self' blob:");
  if (req.path === '/desktop.html' || req.path === '/' || req.path === '/setup.js') res.setHeader('Cache-Control', 'no-store');
...
app.get('/api/update', async (req, res) => res.json({
  ...
  version: '10.30.0',
  channel: 'stable',
  build: 'roadmap hub and release quality batch',
  changelog: [
    'Batch thirty: Roadmap Hub for account security, social, Planner, Vault, sync, AI, ecosystem, accessibility, and release-quality controls.',
    'Batch twenty-nine: Mobile and accessibility controls for high contrast, reduced motion, large text, touch layouts, and keyboard access.',
    'Batch twenty-eight: App ecosystem actions for permissions, updates, trust, install cleanup, and reliability.',
    'Batch twenty-seven: AI workspace paths for cloud, local, and offline privacy choices.',
    'Batch twenty-six: Sync and offline queue status, retry actions, conflict entry points, and recovery controls.',
    'Batch twenty-five: Files and Vault recovery actions with encrypted backup entry points.',
    'Batch twenty-four: Planner reminders and direct access to Board, Agenda, Calendar, and Today.',
    'Batch twenty-three: Friend-only social and call entry points with diagnostics and optional media.',
    'Batch twenty-two: Account security readiness, trusted-device entry points, recovery, and passkey capability detection.',
