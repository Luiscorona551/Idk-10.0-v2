(() => {
  ...
  const rooms = () => Array.isArray(read(ROOMS_KEY, [])) ? read(ROOMS_KEY, []).slice(0, 24) : [];
  ...
  const saveRoom = record => write(ROOMS_KEY, [record, ...rooms().filter(item => item.code !== record.code)].slice(0, 24));
  const removeRoom = value => write(ROOMS_KEY, rooms().filter(item => item.code !== value));
  ...
  function roomApp(options = {}) {
    ...
    let socket = null, record = null, role = 'member', ownerId = null, memberList = [], shared = null;
    ...
    function renderMembers() {
      ...
      members.replaceChildren(...(memberList.length ? memberList.map(member => { const row = document.createElement('article'); row.className = 'idk-room-member'; row.innerHTML = `<span class="idk-room-avatar">${esc((member.name || '?').slice(0, 1).toUpperCase())}</span><div><strong>${esc(member.name || 'Guest')}${member.id === ownerId ? ' · Host' : ''}</strong><small>${member.guest ? 'Guest' : member.role || 'Member'}</small></div><div class="idk-rooms-actions"></div>`; const actions = row.querySelector('.idk-rooms-actions'); if (member.userId && member.userId !== userId()) actions.append(button('Call', () => window.IdkCalls?.start?.({ id: member.userId, name: member.name }) || open('calls'), 'btn tab')); if (isModerator() && member.id !== ownerId && member.id !== socket?.peerId) { actions.append(button('Mute', () => send({ type: 'moderation', action: 'mute', targetId: member.id, minutes: 5 }), 'btn tab')); actions.append(button('Kick', () => send({ type: 'moderation', action: 'kick', targetId: member.id }), 'btn tab')); if (role === 'owner' && member.role === 'member') actions.append(button('Make mod', () => send({ type: 'moderation', action: 'promote', targetId: member.id }), 'btn tab')); } return row; }) : [Object.assign(document.createElement('p'), { className: 'idk-rooms-note', textContent: 'Waiting for people to join.' })]));
    }

    function renderLive() {
      lobby.hidden = true; live.hidden = false; root.querySelector('[data-room-name]').textContent = record.name || `Room ${record.code}`; root.querySelector('[data-room-code]').textContent = record.code; root.querySelector('[data-room-note]').textContent = record.readOnlyGuests ? 'Guest view mode is on.' : 'Members can chat and share selected workspace details.'; renderMessages(); renderMembers();
      compose.querySelector('input').disabled = !canWrite(); compose.querySelector('button').disabled = !canWrite();
    }

    function closeSocket() { socket?.close(); socket = null; }
    function handle(data) {
      if (data.type === 'joined') { role = data.role || 'member'; ownerId = data.ownerId; memberList = data.users || []; record.messages = Array.isArray(data.history) ? data.history : []; record.readOnlyGuests = Boolean(data.config?.readOnlyGuests ?? record.readOnlyGuests); if (data.config?.name && !record.name) record.name = data.config.name; saveRoom(record); status.textContent = `Connected as ${role}.`; renderLive(); return; }
      ...
      if (data.type === 'workspace-share') { shared = data.workspace; root.querySelector('[data-shared-by]').textContent = data.name || 'a member'; root.querySelector('[data-shared-copy]').textContent = `${(data.workspace?.apps || []).length} app(s) and selected room settings are ready.`; root.querySelector('[data-shared]').hidden = false; return; }
      ...
      if (data.type === 'error' || data.type === 'moderation-result' || data.type === 'muted') { status.textContent = data.text || (data.until ? `Muted until ${new Date(data.until).toLocaleTimeString()}.` : 'Room update.'); return; }
      if (data.type === 'kicked') { closeSocket(); lobby.hidden = false; live.hidden = true; lobbyStatus.textContent = data.reason || 'You were removed from the room.'; }
    }

    function connect(next) {
      const normalized = String(next.code || '').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 8); if (!normalized) return;
      closeSocket(); record = { code: normalized, name: String(next.name || `Room ${normalized}`).slice(0, 40), theme: next.theme || 'midnight', readOnlyGuests: Boolean(next.readOnlyGuests), messages: [], updatedAt: Date.now() }; saveRoom(record); renderLive(); status.textContent = 'Connecting...';
      const protocol = location.protocol === 'https:' ? 'wss' : 'ws'; socket = new WebSocket(`${protocol}://${location.host}/chat`); socket.onopen = () => { send({ type: 'join', room: record.code, name: userName(), guestMode: !userId(), roomConfig: next.owner ? { name: record.name, theme: record.theme, readOnlyGuests: record.readOnlyGuests } : undefined }); }; socket.onmessage = event => { try { handle(JSON.parse(event.data)); } catch {} }; socket.onclose = () => { if (record) status.textContent = 'Room disconnected. Reopen to reconnect.'; };
    }

    joinForm.onsubmit = event => { event.preventDefault(); const form = event.currentTarget; const value = { code: form.code.value.trim() || code(), name: form.name.value.trim() || 'IDK Room', theme: form.theme.value, readOnlyGuests: form.readonly.checked, owner: !form.code.value.trim() }; connect(value); };
    root.querySelector('[data-new]').onclick = () => { joinForm.reset(); joinForm.code.value = code(); joinForm.name.value = 'Focus Room'; joinForm.readonly.checked = false; joinForm.querySelector('[name=code]').focus(); };
    ...
    root.querySelector('[data-share-workspace]').onclick = () => { sharePanel.hidden = !sharePanel.hidden; };
    root.querySelector('[data-send-share]').onclick = () => { const workspace = {}; if (root.querySelector('[data-share-apps]').checked) workspace.apps = [...document.querySelectorAll('#windows .window[data-app]:not(.minimized)')].map(win => win.dataset.app).filter(Boolean).slice(0, 12); if (root.querySelector('[data-share-theme]').checked) workspace.theme = read('theme', 'midnight'); if (root.querySelector('[data-share-desktop]').checked) workspace.desktop = localStorage.getItem('idkActiveVirtualDesktop') || 'desktop-1'; send({ type: 'workspace-share', workspace }); status.textContent = 'Selected workspace details shared.'; sharePanel.hidden = true; };
    root.querySelector('[data-apply-share]').onclick = () => { if (!shared) return; if (shared.theme && typeof applyTheme === 'function') applyTheme(shared.theme); if (shared.desktop && window.IDKEcosystem?.showDesktop) window.IDKEcosystem.showDesktop(shared.desktop); (shared.apps || []).forEach((app, index) => setTimeout(() => open(app), index * 100)); notify('IDK Rooms', 'Shared workspace applied locally.', 'success'); };
    root.querySelector('[data-room-actions]').append(button('Leave room', () => { closeSocket(); record = null; live.hidden = true; lobby.hidden = false; status.textContent = 'Not connected'; renderSaved(); }, 'btn tab'), button('Save settings', () => { saveRoom(record); send({ type: 'room-config', config: { name: record.name, theme: record.theme, readOnlyGuests: record.readOnlyGuests } }); notify('IDK Rooms', 'Room settings saved.', 'success'); }, 'btn tab'));
    ...
    renderSaved();
    const hash = location.hash.match(/^#room=([A-Za-z0-9]+)/); if (options.roomCode || hash) connect(rooms().find(item => item.code === (options.roomCode || hash[1]).toUpperCase()) || { code: options.roomCode || hash[1], name: 'Invited Room' });
    root.cleanup = closeSocket;
    return root;
  }
  ...
  window.IDKRooms = { open: options => open('rooms', options), create: () => open('rooms', { roomCode: code() }), saved: rooms };
