...
function verifyPassword(password, stored) {
  ...
  try { return timingSafeEqual(scryptSync(String(password), salt, 64), Buffer.from(hex, 'hex')); } catch { return false; }
}
...
function userFromReq(req) {
  ...
  if (!raw) return null;
  const value = raw.slice('idk_account='.length);
  let decoded = '';
...
function setSession(res, userId) {
  const token = encodeURIComponent(makeToken(userId));
  res.setHeader('Set-Cookie', `idk_account=${token}; Path=/; HttpOnly; SameSite=Lax; Secure; Max-Age=${Math.floor(MAX_AGE / 1000)}`);
}

function clearSession(res) {
  res.setHeader('Set-Cookie', 'idk_account=; Path=/; HttpOnly; SameSite=Lax; Secure; Max-Age=0');
}
...
export function accountRoutes(app) {
  app.get('/api/account/status', async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ ok: false, error: 'Account service is temporarily unavailable.' });
    }
  });
  ...
  app.post('/api/account/login', async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ ok: false, error: 'Could not sign in right now.' });
    }
  });
  ...
  app.get('/api/account/state', async (req, res) => {
    if (!pool) return res.status(503).json({ ok: false, error: 'Persistent database is not configured.' });
    const id = userFromReq(req);
    if (!id) return res.status(401).json({ ok: false, error: 'Not signed in.' });
    try {
      ...
      res.status(500).json({ ok: false, error: 'Could not load your IDK data.' });
    }
  });
  ...
  app.put('/api/account/state', async (req, res) => {
    ...
    if (!id) return res.status(401).json({ ok: false, error: 'Not signed in.' });
    const b = req.body || {};
    const j = (v, d) => JSON.stringify(v === undefined ? d : v);
    try {
      ...
      console.error('Account state write failed:', error);
      res.status(500).json({ ok: false, error: 'Could not save your IDK data.' });
    }
  });
}
