...
function setSession(res, userId) { const token = encodeURIComponent(makeToken(userId)); res.setHeader('Set-Cookie', `idk_account=${token}; Path=/; HttpOnly; SameSite=Lax; Secure; Max-Age=${Math.floor(MAX_AGE / 1000)}`); }
function clearSession(res) { res.setHeader('Set-Cookie', 'idk_account=; Path=/; HttpOnly; SameSite=Lax; Secure; Max-Age=0'); }
