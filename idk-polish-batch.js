(() => {
  ...
  function addWidgets(){
    ...
    const make=(id,opts,x,y)=>{const w=document.createElement('section');w.className='idk-widget';w.dataset.widget=id;w.style.left=(x??saved[id]?.x??opts.x)+'px';w.style.top=(y??saved[id]?.y??opts.y)+'px';w.innerHTML='<header><strong></strong><button type="button" aria-label="Close widget">×</button></header><div></div>';w.querySelector('strong').textContent=opts.title;w.querySelector('div').append(opts.content());const closeButton=w.querySelector('button');const closeWidget=e=>{e.preventDefault();e.stopPropagation();remove(id,w)};closeButton.addEventListener('pointerdown',closeWidget);closeButton.addEventListener('click',closeWidget);let sx=0,sy=0,ox=0,oy=0;w.querySelector('header').onpointerdown=e=>{if(e.target.closest('button'))return;sx=e.clientX;sy=e.clientY;ox=w.offsetLeft;oy=w.offsetTop;w.querySelector('header').setPointerCapture?.(e.pointerId);const move=q=>{w.style.left=Math.max(0,ox+q.clientX-sx)+'px';w.style.top=Math.max(0,oy+q.clientY-sy)+'px'};const up=()=>{w.querySelector('header').onpointermove=null;save(id,w)};w.querySelector('header').onpointermove=move;w.querySelector('header').onpointerup=up};layer.append(w);return w};
