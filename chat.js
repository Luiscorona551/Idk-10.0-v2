  ...
  socket.on('message', raw => {
    ...
    if (data.type === 'direct-message' && socket.code) {
      ...
      if (!targetSocket || targetSocket === socket) {
        return send(socket, { type: 'error', text: 'Choose someone else in this room for a personal chat.' });
      }
      if (!text) return;
      const payload = {
        type: 'message',
        private: true,
        name: socket.nick,
        fromId: socket.peerId,
        toId: targetSocket.peerId,
        toName: targetSocket.nick,
        text,
        at: Date.now()
      };
      send(socket, payload);
