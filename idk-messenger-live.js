(() => {
  ...
  function openMessenger(options = {}) {
    ...
    const state = { users: [], room: '', me: '', meUserId: null, selected: null, selectedUserId: null, selectedName: '', unreadRoom: 0, unreadDm: 0, joined: false, typingTimers: {}, localTyping: { room: false, dm: false } };
    let connectionId = 0;
    ...
    function renderUsers() {
      ...
      visible.forEach(user => { const button = document.createElement('button'); button.type = 'button'; button.className = 'idk-live-member' + ((state.selectedUserId && state.selectedUserId === user.userId) || (!state.selectedUserId && state.selected === user.id) ? ' selected' : ''); button.innerHTML = `<span class="idk-live-dot"></span><span><strong>${esc(user.name)}</strong><small>${user.id === state.selected ? 'Selected' : user.userId === state.meUserId ? 'You' : 'Online'}</small></span>`; if (user.id !== state.selected && user.userId !== state.meUserId) button.onclick = () => selectTarget(user); members.append(button); });
