(() => {
  ...
  const clearLocal = () => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      if (k && !skipKeys.has(k)) keys.push(k);
    }
    keys.forEach(k => { try { localStorage.removeItem(k); } catch {} });
  };
  const restoreLocal = data => {
    if (!data || typeof data !== 'object') return;
    Object.entries(data).forEach(([k, v]) => { try { localStorage.setItem(k, String(v)); } catch {} });
  };
  ...
  let user = null, timer = null, saving = false, restored = false, saveQueued = false;

  function style() {
    if (document.getElementById('idk-account-style')) return;
    const s = document.createElement('style');
    s.id = 'idk-account-style';
  ...
  function profile() {
    if (!user) return;
    style();
    const old = document.getElementById('idk-account-profile');
    if (old) old.remove();
    const p = document.createElement('div');
    p.id = 'idk-account-profile';
    p.innerHTML = `<img src="${esc(user.avatar || 'profile-1.jpg')}" alt=""><span>${esc(user.username)}</span>`;
    document.body.appendChild(p);
  }

  function localPayload() {
    let installed = [], cards = [], sheets = {}, messenger = {};
    try { installed = JSON.parse(localStorage.getItem('idkInstalledPrograms') || '[]'); } catch {}
    try { cards = JSON.parse(localStorage.getItem('idkDesktopCards') || '[]'); } catch {}
    try { sheets = JSON.parse(localStorage.getItem('idkSheetsData') || '{}'); } catch {}
    try { messenger = { chatName: JSON.parse(localStorage.getItem('chatName') || '""'), profile: JSON.parse(localStorage.getItem('idkProfile') || 'null') }; } catch {}
    return { desktop: { localStorage: readLocal() }, games: installed, cards, sheets, messenger };
  }

  async function sync() {
    if (!user || !restored || saving) return false;
    saving = true;
    try { const r = await put('/api/account/state', localPayload()); return Boolean(r.ok); }
    catch { return false; }
    finally { saving = false; }
  }

  async function restore() {
    const r = await get('/api/account/state');
    if (!r.ok) return false;
    const s = r.state || {};
    clearLocal();
    restoreLocal(s.desktop?.localStorage);
  ...
  function watchLocalStorage() {
    ...
    const queue = () => {
      if (!user || !restored || saveQueued) return;
      saveQueued = true;
  ...
  async function auth() {
    let st;
    try { st = await get('/api/account/status'); } catch { return false; }
    if (!st.configured) return true;
    if (st.authenticated) {
      await startUser(st.user);
      return true;
    }
    ...
    const title = o.querySelector('#idk-account-title');
    const copy = o.querySelector('#idk-account-copy');
    const submit = o.querySelector('#idk-account-submit');
    const avatar = o.querySelector('#idk-account-avatar-label');
    let register = false;
    toggle.onclick = () => {
      register = !register;
      title.textContent = register ? 'Create your IDK account' : 'Welcome to IDK 10.0';
      copy.textContent = register ? 'Your personal desktop will be saved securely to your account.' : 'Sign in to restore your personal desktop, games, Files and Messenger data.';
      submit.textContent = register ? 'Create account' : 'Sign in';
      toggle.textContent = register ? 'I already have an account' : 'Create account';
      avatar.hidden = !register;
      form.querySelector('#idk-account-pass').autocomplete = register ? 'new-password' : 'current-password';
    };
    form.onsubmit = async e => {
      e.preventDefault();
      const err = o.querySelector('#idk-account-error');
      err.textContent = '';
      submit.disabled = true;
      const body = { username: o.querySelector('#idk-account-user').value.trim(), password: o.querySelector('#idk-account-pass').value, avatar: o.querySelector('#idk-account-avatar').value.trim() || 'profile-1.jpg' };
      try {
        const r = await post(register ? '/api/account/register' : '/api/account/login', body);
        if (!r.ok) { err.textContent = r.error || 'Could not sign in.'; submit.disabled = false; return; }
        o.remove();
        await startUser(r.user);
      } catch {
        err.textContent = 'Could not connect to the IDK account service.';
