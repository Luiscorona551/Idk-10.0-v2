(() => {
  ...
  async function launchPublicProgram(program) {
    ...
    if (!popup) { notify('App Manager', 'Allow pop-ups for IDK 10.0 to launch this program.'); URL.revokeObjectURL(url); return; }
    popup.opener = null;
  ...
  function openDiscovery() {
    ...
    const render = () => {
      ...
      items.forEach(program => {
        const installed = program.builtin ? window.IDKBuiltInStore?.installed(program.id) : read('idkInstalledPrograms', []).find(item => item.id === program.id);
        const card = document.createElement('article'); card.className = 'idk-discovery-card';
        card.innerHTML = `${program.screenshot ? `<img src="${esc(program.screenshot)}" alt="" loading="lazy">` : '<div class="idk-discovery-art">IDK</div>'}<strong>${esc(program.icon || '[]')} ${esc(program.name)}${program.verified ? ' · Verified' : ''}</strong><small>${esc(program.category)} · v${esc(program.version || '1.0.0')}</small><button type="button" class="idk-discovery-author">by ${esc(program.author || 'IDK creator')}</button><small>${esc(program.description || 'Self-contained HTML app')}</small><small>${program.ratingCount ? `${Number(program.rating || 0).toFixed(1)} / 5 · ${program.ratingCount} rating${program.ratingCount === 1 ? '' : 's'}` : 'No ratings yet'}${program.contentHash ? ` · ${esc(program.contentHash.slice(0, 12))}…` : ''}</small><div class="idk-platform-actions"></div>`;
        const actions = card.querySelector('.idk-platform-actions');
        if (program.builtin) { const open = document.createElement('button'); open.type='button'; open.className='btn tab'; open.dataset.builtinApp=program.id; open.textContent='Open'; open.disabled=!installed; open.addEventListener('click',event=>{ event.preventDefault(); event.stopPropagation(); if(!window.IDKBuiltInStore?.installed(program.id)) return; window.IDKBuiltInStore.open(program.id); }); actions.append(open); } else { const open = document.createElement('a'); open.className = 'btn tab'; open.href = program.contentUrl; open.target = '_blank'; open.rel = 'noopener'; open.textContent = 'Open'; actions.append(open); }
        const install = document.createElement('button'); install.type='button'; install.className='btn'; if(program.builtin){ install.textContent=installed?'Uninstall':'Install'; install.onclick=()=>{ window.IDKBuiltInStore?.setInstalled(program.id,!installed); window.IDKBuiltInStore?.register(); notify('IDK Store',`${program.title} ${installed?'uninstalled':'installed'}. Reloading desktop…`); setTimeout(()=>location.reload(),250); }; } else { install.textContent=installed ? (compareVersion(program.version, installed.version)>0 ? 'Update':'Installed') : 'Install'; install.disabled=install.textContent==='Installed'; install.onclick=()=>installOrUpdate(program,install).then(render); } actions.append(install);
    ...
    catalog().then(value => { const builtins = window.IDKBuiltInStore?.defs?.map(item => ({...item, builtin:true, version:'1.0.0'})) || []; programs = [...builtins, ...value]; [...new Set(programs.map(item => item.category).filter(Boolean))].sort().forEach(item => category.append(new Option(item, item))); status.textContent = `${programs.length} apps available, including built-in free apps.`; updatePendingUpdates(value); render(); }).catch(error => { status.textContent = error.message; grid.replaceChildren(); });
    return root;
  }
  ...
  function runAutomation(routine) {
    ...
    else if (routine.action === 'focus') window.IDKFocus?.start?.(Number(routine.minutes) || 50, routine.mode || 'deep');
