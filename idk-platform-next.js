  ...
  function openDiscovery() {
    ...
    const render = () => {
      ...
      items.forEach(program => {
        ...
        const actions = card.querySelector('.idk-platform-actions');
        if (program.builtin) { const open = document.createElement('button'); open.type='button'; open.className='btn tab'; open.textContent='Open'; open.disabled=!installed; open.onclick=()=>window.OS?.open?.(program.id); actions.append(open); } else { const open = document.createElement('a'); open.className = 'btn tab'; open.href = program.contentUrl; open.target = '_blank'; open.rel = 'noopener'; open.textContent = 'Open'; actions.append(open); }
