(() => {
  ...
  function pane(root, id) {
    ...
    if (id === 'all') {
      ...
      cards.forEach(card => { const cardRoot = document.createElement('article'); cardRoot.className = 'idk-roadmap-card'; cardRoot.innerHTML = '<span>BATCH ' + card[1] + '</span><h3>' + esc(card[2]) + '</h3><p>' + esc(card[3]) + '</p>'; cardRoot.append(button('Open', () => open(card[4]))); grid.append(cardRoot); });
      item.append(grid); root.append(item); return;
    }
    const card = cards.find(value => value[0] === id);
    if (!card) return pane(root, 'all');
    const item = section(card[2], card[3]);
    ...
    else if (id === 'ai') { const select = document.createElement('select'); select.className = 'field'; ['cloud', 'local', 'offline'].forEach(value => select.append(new Option(value + ' AI', value))); select.value = window.IDKAIControls?.getMode?.() || read('idkAIMode', 'cloud'); select.onchange = () => { window.IDKAIControls?.setMode?.(select.value); write('idkAIMode', select.value); }; item.append(select, button('Open selected mode', () => window.IDKAIControls?.openSelected?.() || open('aiModes')), button('Privacy Center', () => open('privacy'))); }
  ...
  function hub() {
    const root = document.createElement('div'); root.className = 'app idk-roadmap-hub'; root.innerHTML = '<header class="idk-roadmap-header"><div><span class="idk-roadmap-kicker">IDK RELEASE TRAIN · BATCHES 22-30</span><h2>Roadmap Hub</h2><p>Planned updates connected to the features already shipped.</p></div><button class="btn tab" type="button" data-refresh>Refresh</button></header><nav class="idk-roadmap-tabs" role="tablist"></nav><main data-pane></main>';
    const nav = root.querySelector('[role="tablist"]'), main = root.querySelector('[data-pane]');
    ['all', ...cards.map(card => card[0])].forEach(id => { const item = document.createElement('button'); item.type = 'button'; item.dataset.tab = id; item.textContent = id === 'all' ? 'All batches' : cards.find(card => card[0] === id)[2]; item.onclick = () => { nav.querySelectorAll('button').forEach(buttonItem => buttonItem.classList.toggle('active', buttonItem === item)); pane(main, id); }; if (id === 'all') item.classList.add('active'); nav.append(item); });
    root.querySelector('[data-refresh]').onclick = () => pane(main, nav.querySelector('.active')?.dataset.tab || 'all'); pane(main, 'all'); return root;
  }
