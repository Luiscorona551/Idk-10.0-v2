(() => {
  ...
  function callsApp(options = {}) {
    const initialTarget = options.target || null;
    const root = document.createElement('div'); root.id = 'idk-calls-app'; root.className = 'app idk-calls-app';
    root.innerHTML = '<header class="idk-control-head"><div><span class="idk-flow-kicker">IDK CALLS</span><h2>Voice calls</h2><p>Call accepted friends directly from IDK. Audio is peer-to-peer, microphone access is requested only when you start or accept a call, and calls are not recorded.</p></div><span class="idk-control-badge">VOICE FIRST</span></header><div class="idk-call-status" data-status>Loading your friends…</div><section class="idk-call-current" data-current hidden><div class="idk-call-peer"><span class="idk-call-avatar" data-avatar>?</span><div><strong data-peer>Not connected</strong><small data-call-state>Preparing voice call</small></div></div><audio data-audio autoplay></audio><div class="idk-call-actions"><button class="btn" data-mute>Mute</button><button class="btn danger" data-end>End call</button></div></section><section class="idk-call-incoming" data-incoming hidden><strong data-incoming-title>Incoming call</strong><p data-incoming-copy></p><div class="idk-call-actions"><button class="btn" data-accept>Accept voice call</button><button class="btn tab" data-reject>Decline</button></div></section><section class="idk-call-section"><div class="idk-call-section-head"><strong>Friends</strong><button class="btn tab" data-reload>Refresh</button></div><div class="idk-call-friends" data-friends></div></section><section class="idk-call-section"><div class="idk-call-section-head"><strong>Recent calls</strong><small>No cloud call history is stored.</small></div><div class="idk-call-history" data-history></div></section>';
    const status = root.querySelector('[data-status]'), friendsList = root.querySelector('[data-friends]'), current = root.querySelector('[data-current]'), incoming = root.querySelector('[data-incoming]'), audio = root.querySelector('[data-audio]'), history = root.querySelector('[data-history]');
    let friends = [], socket = null, socketReady = null, pc = null, stream = null, call = null, connected = false;
    const setStatus = value => { status.textContent = value; };
    const record = entry => { const items = read('idkCallHistory', []); write('idkCallHistory', [{ ...entry, at: Date.now() }, ...items].slice(0, 20)); renderHistory(); };
    ...
    const clean = () => { pc?.close(); pc = null; stream?.getTracks().forEach(track => track.stop()); stream = null; audio.srcObject = null; connected = false; call = null; renderCall(); };
    ...
    const start = async target => {
      ...
      try { await ensureSocket(); await ensureMedia(); call = { id: `${Date.now()}-${Math.random().toString(36).slice(2)}`, target: { id: friend.id, name: friend.username || friend.name || 'Friend' }, incoming: false }; createPeer(); renderCall(); const offer = await pc.createOffer(); await pc.setLocalDescription(offer); sendCall('invite', { sdp: pc.localDescription }); setStatus(`Calling ${call.target.name}…`); } catch (error) { clean(); setStatus(error.message || 'Could not start the call.'); }
    };
    const accept = async () => { if (!call?.incoming) return; try { await ensureSocket(); await ensureMedia(); createPeer(); await pc.setRemoteDescription(call.offer); call.incoming = false; renderCall(); const answer = await pc.createAnswer(); await pc.setLocalDescription(answer); sendCall('accept', { sdp: pc.localDescription }); setStatus(`Connecting to ${call.target.name}…`); } catch (error) { setStatus(error.message || 'Could not accept the call.'); clean(); } };
    const end = (remote = false) => { if (!call) return; if (!remote) sendCall('end'); if (!connected && call.incoming) record({ name: call.target.name, missed: true, direction: 'incoming' }); clean(); setStatus(remote ? 'Call ended.' : 'Call ended.'); };
    function handleSignal(data) {
      if (data.type === 'call' && data.action === 'invite') { if (!friends.some(item => item.id === data.fromUserId)) return; call = { id: data.callId, target: { id: data.fromUserId, name: data.fromName || 'Friend' }, incoming: true, offer: data.payload?.sdp }; renderCall(); notify('Incoming IDK call', `${data.fromName || 'A friend'} is calling you.`, 'success'); return; }
