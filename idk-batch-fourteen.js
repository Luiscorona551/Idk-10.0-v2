(() => {
  ...
  const diagnostic = (type, detail = {}) => window.IDKCallRuntime?.record?.(type, detail);
  ...
  function callsApp(options = {}) {
    ...
    const playRemoteAudio = async () => {
      ...
      try {
        await audio.play();
        if (enableAudio) enableAudio.hidden = true;
        return true;
      } catch (error) {
        if (enableAudio) enableAudio.hidden = false;
        diagnostic('remote-audio-blocked', { message: error?.message || 'Playback was blocked.' });
        return false;
      }
    };
    const attachRemoteMedia = event => {
      ...
      playRemoteAudio();
      diagnostic('remote-track', { video: Boolean(remote.getVideoTracks?.().length), audio: Boolean(remote.getAudioTracks?.().length) });
    };
    ...
    const ensureSocket = () => {
      ...
      socketReady = new Promise((resolve, reject) => {
        ...
        socket.onopen = () => { diagnostic('socket-open'); reconnectAttempts = 0; socket.send(JSON.stringify({ type: 'join', name: window.IDKAccount?.user?.username || read('idkMessengerProfile', {}).name || 'IDK user', room: read('idkMessengerProfile', {}).room || 'general' })); resolve(); };
        socket.onerror = () => { diagnostic('socket-error'); reject(new Error('The call service is unavailable.')); };
        socket.onclose = event => { diagnostic('socket-close', { code: event.code, reason: event.reason || '' }); socketReady = null; if (!call) return; setStatus('Call service disconnected. Reconnecting…'); if (reconnectAttempts < 3) { reconnectAttempts += 1; reconnectTimer = setTimeout(() => ensureSocket().then(() => { if (call && !connected && call.offer) sendCall('invite', { sdp: call.offer }); }).catch(() => {}), reconnectAttempts * 1200); } };
    ...
    const start = async target => {
      ...
      try {
        ...
        await window.IDKCallRuntime?.loadCallConfig?.();
        await ensureSocket();
        await ensureMedia();
     ...
     const accept = async () => { if (!call?.incoming) return; try { videoEnabled = videoPreference.checked; await window.IDKCallRuntime?.loadCallConfig?.(); await ensureSocket(); await ensureMedia(); call.signalReady = false; call.localCandidates = []; createPeer(); pc.addEventListener('track', attachRemoteMedia); pc.addEventListener('signalingstatechange', flushCandidates); pc.onicecandidate = event => { if (event.candidate) { if (call.signalReady) sendCall('signal', { candidate: event.candidate }); else call.localCandidates.push(event.candidate); } }; await pc.setRemoteDescription(call.offer); flushCandidates(); call.incoming = false; call.direction = 'incoming'; call.startedAt = Date.now(); diagnostic('call-accept', { video: videoEnabled }); renderCall(); const answer = await pc.createAnswer(); await pc.setLocalDescription(answer); sendCall('accept', { sdp: pc.localDescription }); call.signalReady = true; call.localCandidates.splice(0).forEach(candidate => sendCall('signal', { candidate })); setStatus(`Connecting to ${call.target.name}…`); } catch (error) { diagnostic('call-accept-failed', { message: error?.message || 'unknown' }); setStatus(error.message || 'Could not accept the call.'); clean(); } };
    ...
    const renderFriends = () => { const query = friendSearch.value.trim().toLowerCase(); const visible = friends.filter(friend => !query || String(friend.username || friend.name || '').toLowerCase().includes(query)); friendsList.replaceChildren(...(visible.length ? visible.map(friend => { const row = document.createElement('div'); row.className = 'idk-call-friend'; row.dataset.friendId = friend.id; row.innerHTML = `<span class="idk-call-avatar">${esc((friend.username || '?').slice(0, 1).toUpperCase())}</span><div class="idk-call-friend-copy"><strong>${esc(friend.username || friend.name || 'Friend')}</strong><small>Available for an IDK call</small></div>`; row.append(button('Start call', () => start(friend), 'btn')); return row; }) : [Object.assign(document.createElement('p'), { className: 'idk-control-status', textContent: query ? 'No accepted friend matches that search.' : 'No accepted friends yet. Add friends first.' })])); };
    const load = async () => { try { const data = await fetch('/api/friends', { credentials: 'same-origin' }).then(response => response.json()); if (!data.ok) throw new Error(data.error || 'Sign in to use IDK Calls.'); friends = data.friends || []; renderFriends(); setStatus(friends.length ? `${friends.length} accepted friend${friends.length === 1 ? '' : 's'} available.` : 'No accepted friends yet.'); if (initialInvite) handleSignal(initialInvite); if (initialTarget) start(initialTarget); } catch (error) { friends = []; renderFriends(); friendsList.replaceChildren(Object.assign(document.createElement('p'), { className: 'idk-control-status', textContent: error.message })); setStatus(error.message); } };
    if (enableAudio) enableAudio.onclick = async () => { const ok = await playRemoteAudio(); if (ok) setStatus('Audio enabled · you can hear your friend now.'); };
    root.querySelector('[data-accept]').onclick = accept; root.querySelector('[data-reject]').onclick = () => { if (call) { sendCall('reject'); record({ name: call.target.name, missed: true, direction: 'incoming' }); } clean(); setStatus('Call declined.'); }; root.querySelector('[data-end]').onclick = () => end(); root.querySelector('[data-mute]').onclick = event => { const muted = stream?.getAudioTracks().some(track => !track.enabled); stream?.getAudioTracks().forEach(track => { track.enabled = muted; }); event.currentTarget.textContent = muted ? 'Mute mic' : 'Unmute mic'; }; root.querySelector('[data-video-toggle]').onclick = event => { const tracks = stream?.getVideoTracks() || []; if (!tracks.length) return setStatus('Start the next call with video enabled.'); const enabled = tracks.some(track => track.enabled); tracks.forEach(track => { track.enabled = !enabled; }); event.currentTarget.textContent = enabled ? 'Turn video on' : 'Turn video off'; }; videoPreference.onchange = () => { videoEnabled = videoPreference.checked; }; friendSearch.oninput = renderFriends; root.querySelector('[data-reload]').onclick = load;
    renderHistory(); load();
