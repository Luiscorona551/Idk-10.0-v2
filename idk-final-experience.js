(() => {
  ...
  function openPublisher() {
    ...
    root.innerHTML = '<div class="idk-publish-card"><header class="idk-publish-head"><strong>Publish to IDK App Store</strong><button type="button" data-close aria-label="Close">×</button></header><form class="idk-publish-form"><p class="idk-publish-note">Share a self-contained HTML app or game with friends. Published programs run in a sandboxed tab.</p><label>Name<input name="name" required maxlength="100" placeholder="My IDK game"></label><label>Category<select name="category"><option>Games</option><option>Productivity</option><option>Creative</option><option>Other</option></select></label><label>Version<input name="version" maxlength="24" value="1.0.0"></label><label>Icon<input name="icon" maxlength="8" value="🧩" aria-label="Program icon"></label><label>Author<input name="author" maxlength="64"></label><label>Screenshot URL<input name="screenshot" type="url" maxlength="500" placeholder="https://…"></label><label>HTML source<textarea name="content" required spellcheck="false" placeholder="<!doctype html>..."></textarea></label><div class="idk-publish-actions"><button class="btn" type="submit">Publish</button><button class="btn tab" type="button" data-refresh>Refresh public catalog</button></div><p class="idk-publish-status" data-status></p><div class="idk-publish-catalog" data-catalog></div></form></div>';
    ...
    const renderCatalog = programs => {
      ...
      programs.forEach(program => {
        ...
        card.innerHTML = `${program.screenshot ? `<img src="${esc(program.screenshot)}" alt="" loading="lazy">` : ''}<strong>${esc(program.icon)} ${esc(program.name)}</strong><small>${esc(program.category)} · v${esc(program.version)} · by ${esc(program.author)}</small><small>${program.ratingCount ? `${program.rating.toFixed(1)} / 5 · ${program.ratingCount} rating${program.ratingCount === 1 ? '' : 's'}` : 'No ratings yet'}</small>`;
